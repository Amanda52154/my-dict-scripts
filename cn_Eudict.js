class cn_Eudict {
  ...
  async fetchAndParseDetail(url) {
    ...
    content.innerHTML = content.innerHTML
      .replace(/<p class="exp">(.+?)<\/p>/gi, '<span class="exp">$1</span>')
      .replace(/<span class="exp"><br>/gi, '<span class="exp">')
      .replace(/<span class="eg"><br>/gi, '<span class="eg">');
  ...
  async findTerm(word) {
    ...
    try {
      ...
      const filtered = terms
        .filter(t => t.value && t.recordid && t.recordtype !== 'CG')
        .slice(0, this.maxexample);
